...
real(8):: V_wind, SolarAlt, pi, skytemp, hour, P_amb, T_db, T_air, T_dp, T_sky,I_b, SolarAz, fluid, m_dot_htf, m_dot_htfX,& 
          ...
          opteff_des, I_bn_des, Ap_tot, L_tot, loss_tot, Pipe_hl_coef, SCA_drives_elec,LU_diam,&
          ...
          defocus_new,defocus_old,solar_alt_des
...
if (info(7) .eq. -1) then
    ...
    x1=0.d0; x2=0.d0; opteff_des=0.d0; m_dot_design=0.d0; L_tot=0.d0; loss_tot=0.d0
    do i=1,nSCA
        CT = SCAInfoArray(i,2)
    ...
    do i=1,nSCA
        ...
        !2/21/13 twn, equation to solve for cos(theta) requires solar altitude angle, not zenith angle
        !solar_alt_des = pi/2. - (lattitude - 23.5*pi/180)        
        !x1 = sqrt(1. - (cos(solar_alt_des - ColTilt) - cos(ColTilt)*cos(solar_alt_des)*(1.0-cos(0. - ColAz)))**2 )
        ...
        do j=1,nHCEvar
            ...
            !2/21/13, twn: remove end loss from optical efficiency at design
            opteff_des = opteff_des+ Shadowing(HT,j)*TrackingError(CT)*GeomEffects(CT)*Rho_mirror_clean(CT)*Dirt_mirror(CT)*&
                                     Dirt_HCE(HT,j)*Error(CT)*(L_actSCA(CT)/L_tot)*HCE_FieldFrac(HT,j)
...
DO WHILE(((abs(diff_T3)>T3_tol).and.(qq<100)).or.(qq<2))    !Outer loop: Find T_3 such than energy balance is satisfied
    ...
    IF (glazingIntact) THEN
        ...
        DO WHILE((abs(diff_q5)>q5_tol_1).and.(q5_iter<100))       !Determine T_4 such that energy balance from T_3 to surroundings is satisfied
            ...
            diff_q5 = (q_5out - q_45cond)/abs(q_45cond)     ![W/m] 11/18/13, twn: when q_45_cond is negative and q_5out is positive, this equation was returning the wrong sign
    ...
    DO WHILE((abs(diff_T2)>T2_tol).and.(q_conv_iter<100))
        ...
        ! 11/18/13 twn: Don't let T_2 become negative
        T_2 =  max( 10.0, fT_2(q_12conv, T_1_ave, T_2g, Fluid, D_2, D_p, D_h, v_1, Flow_type, .true.) )  !Calculate T_2 (with previous T_2 as input)
...
!    - hour  -> the hour, in solar time from midnight                 *
